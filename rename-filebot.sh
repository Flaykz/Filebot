...
NUMBER_LINE_TO_SCAN=150
LINE_LOG=`tail -n $NUMBER_LINE_TO_SCAN "$FILELOG" |grep -n -F "********************************************************"|tail -n 1|cut -d: -f1`
NUMBER_LINE=`expr $NUMBER_LINE_TO_SCAN - $LINE_LOG`

if ! [ -z `tail -n $NUMBER_LINE_TO_SCAN "$FILELOG" |tail -n $NUMBER_LINE |grep "already exists"` ] > /dev/null 2>&1
then
        #fichier deja existant
        #tester si c'est du mkv ou du mp4 = facilement modifiable
        TR_TORRENT_LENGTH=`ls -sh "$TR_TORRENT_DIR/$TR_TORRENT_NAME"|awk '{print $1}'`
        EXISTING_FILE=`tail -n $NUMBER_LINE_TO_SCAN "$FILELOG" |tail -n $NUMBER_LINE |grep "already exists" |sed -E 's/(.*)\] because \[(.*)(\] already exists)/\2/' |tail -n 1`
        EXISTING_FILE_LENGTH=`ls -sh "$EXISTING_FILE"|awk '{print $1}'`
        ...
        then
                ...
                echo $(date +"%d-%m-%y %T") "Suppression de $TR_TORRENT_DIR/$TR_TORRENT_NAME : $TR_TORRENT_LENGTH (Ancien : $EXISTING_FILE_LENGTH)" 1>&2 >> $FILETOSEND
        else
                #A voir pour tester si c'est du HD ou pas, lequel faut remplacer etc
                echo $(date +"%d-%m-%y %T") "Action manuelle requise, nouveau fichier : $TR_TORRENT_DIR/$TR_TORRENT_NAME, taille $TR_TORRENT_LENGTH | ancien fichier : $EXISTING_FILE, taille $EXISTING_FILE_LENGTH" >> "$FILELOG"
                echo $(date +"%d-%m-%y %T") "Action manuelle requise, nouveau fichier : $TR_TORRENT_DIR/$TR_TORRENT_NAME, taille $TR_TORRENT_LENGTH | ancien fichier : $EXISTING_FILE, taille $EXISTING_FILE_LENGTH" 1>&2 >> $FILETOSEND
        fi
else
        #On modifie les droits si un fichier est rajouté
        if ! [ -z `tail -n $NUMBER_LINE_TO_SCAN "$FILELOG" |tail -n $NUMBER_LINE |grep -F "[MOVE] From"` ] > /dev/null 2>&1
        then
                tail -n $NUMBER_LINE_TO_SCAN "$FILELOG" |tail -n $NUMBER_LINE |grep -F "[MOVE] From" 1>&2 >> $FILETOSEND
                #On modifie les droits sur les fichiers téléchargés
                DIR_PATH=`tail -n $NUMBER_LINE_TO_SCAN "$FILELOG" |tail -n $NUMBER_LINE |grep -F "[MOVE] From" |sed -E 's/(.*)\] to \[(.*)(\])/\2/'`
                ...
                echo $(date +"%d-%m-%y %T") "Aucun fichier déplacé : " `tail -n $NUMBER_LINE_TO_SCAN "$FILELOG" |tail -n $NUMBER_LINE` 1>&2 >> $FILETOSEND
        fi
fi
